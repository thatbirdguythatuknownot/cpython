...
There are now increment and decrement operators (``++`` and ``--``, has
prefix/postfix versions) and their magic methods ``__inc__`` and ``__dec__``.
Added new opcodes :opcode:`UNARY_INCREMENT` and :opcode:`UNARY_DECREMENT`
for the operations. See :pep:`9999` for more details. (Contributed by
Jeremiah Vivian (Pascual) in :issue:`99999`.)
...
* New functions ``operator.increment`` and ``operator.decrement``
  added such that ``operator.increment(obj) == ++obj`` and
  ``operator.decrement(obj) == --obj``. (Contributed by
  Jeremiah Vivian (Pascual) in :issue:`99999`.)
...
* Add :opcode:`UNARY_INCREMENT` and :opcode:`UNARY_DECREMENT` opcodes to
  implement incrementing/decrementing respectively.
...
* Add new C API functions :cfunc:`PyNumber_Increment` and
  :cfunc:`PyNumber_Decrement` in ``Objects/abstract.c`` to implement the
